...
fn add32_imm(insn: &Insn, sbpf_version: SBPFVersion) -> Option<String> {
    rust_expr!(format!(
        ...
        if sbpf_version < SBPFVersion::V2 { "i32 as i64 as u64" } else { "u64" },
...
fn mov32_reg(insn: &Insn, sbpf_version: SBPFVersion) -> Option<String> {
    rust_expr!(format!(
        "r{d} = r{s} as {}",
        if sbpf_version < SBPFVersion::V2 { "u32 as u64" } else { "i32 as i64 as u64" },
        d = insn.dst, s = insn.src
    ))
}
...
fn or64_imm(insn: &Insn) -> Option<String> {
    rust_expr!(format!("r{d} |= {i}   ///  r{d} = r{d}.or({i})", d = insn.dst, i = insn.imm))
}

fn or64_reg(insn: &Insn) -> Option<String> {
    rust_expr!(format!("r{d} |= r{s}   ///  r{d} = r{d}.or(r{s})", d = insn.dst, s = insn.src))
}

fn and64_imm(insn: &Insn) -> Option<String> {
    rust_expr!(format!("r{d} &= {i}   ///  r{d} = r{d}.and({i})", d = insn.dst, i = insn.imm))
}

fn and64_reg(insn: &Insn) -> Option<String> {
    rust_expr!(format!("r{d} &= r{s}   ///  r{d} = r{d}.and(r{s})", d = insn.dst, s = insn.src))
}

fn xor64_imm(insn: &Insn) -> Option<String> {
    rust_expr!(format!("r{d} ^= {i}   ///  r{d} = r{d}.xor({i})", d = insn.dst, i = insn.imm))
}

fn xor64_reg(insn: &Insn) -> Option<String> {
    rust_expr!(format!("r{d} ^= r{s}   ///  r{d} = r{d}.xor(r{s})", d = insn.dst, s = insn.src))
}

fn lsh64_imm(insn: &Insn) -> Option<String> {
    rust_expr!(format!("r{d} <<= {i}   ///  r{d} = r{d}.wrapping_shl({i})", d = insn.dst, i = insn.imm))
}
...
fn rsh64_imm(insn: &Insn) -> Option<String> {
    rust_expr!(format!("r{d} >>= {i}   ///  r{d} = r{d}.wrapping_shr({i})", d = insn.dst, i = insn.imm))
}
...
fn jeq_reg(insn: &Insn) -> Option<String> {
    rust_expr!(format!(
        "if r{d} == r{s} {{ pc += {o} }}",
        d = insn.dst, s = insn.src, o = insn.off
    ))
}
